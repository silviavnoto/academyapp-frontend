 ...
 ngOnInit() {
  ...
  getClase(id: number): void {
    this.oClaseService.get(id).subscribe({
      next: (data: IClase) => {
        /*if (!data.participa) {
          alert("El participa es nulo en esta clase");
        } else {
          this.oClase = data;
      ...
      error: (error) => {
        ...
        alert('No se pudo obtener la clase. Verifica que tenga un participa asignado.');
      }
    });
  }
 ...
 /* updateForm() {
  ...
  /*updateForm() {
    ...
    updateForm() {
      ...
      this.oClaseForm?.controls['precio'].setValue('');
      this.oClaseForm?.controls['hora'].setValue('');
      this.oClaseForm?.controls['participa'].setValue({
        id: null,
        id_usuario: null,
        id_clase: null,
      });
